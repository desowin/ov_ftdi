...
class OutputPcap:
    ...
    """OpenVizsla pcap packet format

    First octet in record denotes the record type.
    Currently only the 0xA0 is defined. Other type values are reserved for future use.

    0        1        2        3        4        5        6        7        8
     01234567 01234567 01234567 01234567 01234567 01234567 01234567 01234567
    +--------+-----------------+-----------------+--------------------------+----------~
    |  0xA0  |      Flags      |       Size      |         Timestamp        |  USB Data
    +--------+-----------------+-----------------+--------------------------+----------~

    All multibyte fields are Little-Endian.
    Flags are the same as FPGA uses.
    Size is the length of optional USB Data (may be 0).
    Timestamp is 24-bit 60 MHz counter value.
    Optional USB Data, if present, starts with USB Packet ID.
    """

    def __init__(self, output):
        self.output = output
        self.output.write(struct.pack("IHHIIII", 0xa1b23c4d, 2, 4, 0, 0, 65535, self.LINKTYPE_OPENVIZSLA))
    ...
    def handle_usb(self, ts, pkt, flags):
        ...
        # contain different incl_len (len(pkt) + 8) and orig_len (untruncated packet size + 8)
        self.output.write(struct.pack("IIII", self.utc_ts, nanosec, len(pkt) + 8, len(pkt) + 8))
        # OpenVizsla flags are always encoded as Little-Endian inside the record
        self.output.write(struct.pack("<BHHHB", 0xA0, flags, len(pkt), ts & 0xFFFF, (ts >> 16) & 0xFF))
